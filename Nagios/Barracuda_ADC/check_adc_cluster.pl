...
my $cluster_status    = ".1.3.6.1.4.1.20632.5.99.1.4.7.0";  # 
my @barr_oids		= ($cluster_status);
...
sub check_options {
    ...
    if (! defined($o_crit))
 	{ print "This script needs a critical value equal to the status that it SHOULD be!\n"; print_usage(); exit $ERRORS{"UNKNOWN"}}
...
if (defined ($o_netsnmp)) {
 ...
 my $cluster=undef;
 $cluster = $$resultat{$cluster_status};

 my $n_status="OK";
 my $n_output="This node is curently running as '$cluster'.";

 if ((defined ($o_crit))&&($o_crit ne $cluster)) {
    $n_output .= "  Something is wrong. It should be '$o_crit'." ;
    $n_status="CRITICAL";
    }
