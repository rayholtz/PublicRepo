...
# SNMP Datas
# TrippLite ATS source input
my $ats_source          = ".1.3.6.1.4.1.850.1.1.3.4.3.1.1.1.12.1";
my @array_of_oids       = ($ats_source);
...
if (defined ($o_netsnmp)) {
 ...
 switch($source) {
    case "1"   { $n_output = "Only the primary source input is active."; $n_status="CRITICAL";  }
    case "2"   { $n_output = "Only the secondary source input is active."; $n_status="CRITICAL"; }
    case "3"   { $n_output = "Both primary and secondary source inputs are active."; $n_status="OK"; }
