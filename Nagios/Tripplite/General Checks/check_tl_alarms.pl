...
my $ups_alarms          = ".1.3.6.1.4.1.850.1.3.1.0";
my @array_of_oids       = ($ups_alarms);
...
sub check_options {
   Getopt::Long::Configure ("bundling");
    ...
    if (!defined($o_crit))
        {print "Check needs a critical value!\n"; print_usage(); exit $ERRORS{"UNKNOWN"}}
...
########### TrippLite UPS Battery Alarms check ############
if (defined ($o_netsnmp)) {
 ...
 my $alarms="BAD_DATA";
 $alarms = $$resultat{$ups_alarms};
 #print "alarms is *$alarms*\n";  #testing
 my $n_output="Currently $alarms alarms.";
 my $n_status="OK";

 if ($alarms eq "BAD_DATA") {
    my $n_status="CRITICAL";
 } else {
    my $n_status="OK";
 }

 if (($o_crit!=0)&&($o_crit <= $alarms)) {
    $n_output = "$alarms ALARMS On Device!";
    $n_status="CRITICAL";
    } 
