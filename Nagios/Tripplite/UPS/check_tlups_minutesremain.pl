...
my $ups_minremain          = ".1.3.6.1.4.1.850.1.1.3.1.3.1.1.1.3.1";
my @array_of_oids       = ($ups_minremain);
...
########### TrippLite UPS Battery Minutes Remain Check ############
if (defined ($o_netsnmp)) {
 ...
 my $minutes="BAD_DATA";
 $minutes = $$resultat{$ups_minremain};
 #print "minutes is *$minutes*\n";  #testing
 my $n_output="Currently $minutes minutes of battery use remain.";
 my $n_status="OK";

 if ($minutes eq "BAD_DATA") {
    my $n_status="CRITICAL";
 } else {
    my $n_status="OK";
 }

 if (($o_crit!=0)&&($o_crit >= $minutes)) {
    $n_output = "Only $minutes minutes of battery use left!";
    $n_status="CRITICAL";
    } else {
 if (($o_warn!=0)&&(($minutes >= $o_crit)&&($o_warn >= $minutes))) {
    $n_output = "Only $minutes minutes of battery use left!";
    $n_status="WARNING";
    }
 }
