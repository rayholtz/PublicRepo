...
#################################################################
#
# Help : ./check_tlups_seconbatt.pl -h
#
...
my $ups_seconbatt          = ".1.3.6.1.4.1.850.1.1.3.1.3.1.1.1.2.1";
my @array_of_oids       = ($ups_seconbatt);
...
sub help {
   print "\nSNMP TrippLite ATS Source check ",$Version," for Nagios\n";
   print "This check was written by Ray Holtz\n";
   print "ORIGINALLY A SCRIPT by: '(c)2004-2006 to my cat Ratoune - Author: Patrick Proy'\n\n";
   print_usage();
   print <<EOT;
 -v, --verbose
...
sub check_options {
   Getopt::Long::Configure ("bundling");
    ...
    if (!defined($o_crit))
        {print "Enter a critical value!\n"; print_usage(); exit $ERRORS{"UNKNOWN"}}
        #}else { print "Warning and Critical **ARE** defined\n";}
    if (defined($o_netsnmp)) 
        {if (isnnum($o_crit))
           {print "Need numeric value for critical!\n";print_usage(); exit $ERRORS{"UNKNOWN"} }
        #if (($o_crit != 0) && ($o_warn < $o_crit)) 
        #    {print "warning <= critical! Critical should be less than Warning! \n";print_usage(); exit $ERRORS{"UNKNOWN"}}
    }
}
########## MAIN #######
check_options();
...
########### TrippLite UPS Battery seconds On Battery Check ############
if (defined ($o_netsnmp)) {

  # Get NetSNMP memory values
  $resultat = (Net::SNMP->VERSION lt 4) ?
		$session->get_request(@array_of_oids)
		:$session->get_request(-varbindlist => \@array_of_oids);
 ...
 my $seconds="BAD_DATA";
 $seconds = $$resultat{$ups_seconbatt};
 #print "seconds is *$seconds*\n";  #testing
 my $n_output="Currently running $seconds seconds on battery.";
 my $n_status="OK";

 if ($seconds eq "BAD_DATA") {
    my $n_status="CRITICAL";
 } else {
    my $n_status="OK";
 }

 if (($o_crit!=0)&&($o_crit <= $seconds)) {
    $n_output = "Running on battery TOO LONG! $seconds seconds of battery use so far!";
    $n_status="CRITICAL";
    } 
